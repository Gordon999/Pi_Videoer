...
while True:
    ...
    if show == 0:
        ...
        if col_filter < 4 and (preview == 1 or col_timer > 0) and zoom == 0:
        ...
        if np.shape(gray) == np.shape(oldimg):
            ...
            if ram_frames > auto_limit and time.monotonic() - last > auto_time and auto_save == 1:
              try:
                ...
                if menu == -1 :
                    text(0,0,3,1,1,vf,14,7)
                # if sd card space low move jpgs to USB if present
                free = (os.statvfs('/'))
                used_storage = ((1 - (free.f_bavail / free.f_blocks)) * 100)
                if used_storage > storage_limit :
                    ...
                    USB_Files  = (os.listdir("/media/" + h_user[0] ))
                    if len(USB_Files) > 0:
                        text(0,0,8,0,1,"CAPTURE",16,7)
                        zupics = glob.glob('/home/' + h_user[0] + '/*.jpg')
                        zupics.sort()
                        for xx in range(0,len(zupics)):
                            shutil.move(zupics[xx],'/media/' + h_user[0] + "/" + USB_Files[0] + "/Pictures/")
                            if menu == -1:
            ...
            if ((sar5/diff) * 100 > detection and (sar5/diff) * 100 < det_high) or (time.monotonic() - timer10 > interval and timer10 != 0 and threshold == 0) or record == 1 :
                ...
                if Capture == 1 or record == 1:
                    ...
                    while time.monotonic() - start < v_length/1000 and freeram > ram_limit:
                        ...
                        st = os.statvfs("/run/shm/")
                        freeram = (st.f_bavail * st.f_frsize)/1100000
                        ss = str(int(freeram)) + "MB - " + str(int(used_storage)) + "%"
                        if record == 0:
                            text(0,1,6,1,1,ss,12,3)
                        else:
                            text(0,1,6,1,1,ss,12,0)
                    ...
                    if used_storage > storage_limit and len(USB_Files) > 0 :
                        ...
                        if len(USB_Files) > 0:
                            ...
                            for xx in range(len(zpics)-1,-1,-1):
                                shutil.move(zpics[xx],'/media/' + h_user[0] + "/" + USB_Files[0] + "/Pictures/")
                            ram_frames = 0
                            frames = 0
                            vf = str(ram_frames) + " - " + str(frames)
                            if menu == -1:
                    ...
                    if used_storage > storage_limit and len(USB_Files) == 0 :
                        ...
                        if trace == 1:
                            print ("Step 12a sd card limit exceeded, no USB stick")
                        Capture = 0
                        if menu == -1:
                            Capture = 0
                            button(0,0,0)
            ...
            # keys   
                ...
                if g == 0 and menu == -1 :
                  ...
                  if len(Sideos) > 0:
                    ...
                    for x in range(0,len(Sideos)):
                        if len(Sideos[x]) > 32:
                            if Sideos[x][18+(l_len-2):30+(l_len-2)] != z:
                                z = Sideos[x][18+(l_len-2):30+(l_len-2)]
                                outvids.append(z)
                        else:
                            if Sideos[x][9:21] != y:
                                y = Sideos[x][9:21]
