...
fan_time      = 10      # temperature sampling time in seconds *
fan_low       = 65      # fan OFF below this, 25% to 100% pwm above this *
fan_high      = 78      # fan 100% pwm above this *
sd_hour       = 19      # Shutdown Hour, 1 - 23, 0 will NOT SHUTDOWN *
...
timer10       = 0
col_filterp   = 0
config_file   = "PiVideoconfig001.txt"
ram_snaps     = 0
a             = int(scr_width/3)
b             = int(scr_height/2)
fcount        = 0
...
vwidths       = [1280,1456,1536,1920]
vheights      = [ 720,1088, 864,1080]
...
# check PiVideoconfigXXX.txt exists, if not then write default values
if not os.path.exists(config_file):
    points = [h_crop,threshold,fps,mode,speed,gain,brightness,contrast,Capture,preview,awb,detection,int(red*10),int(blue*10),
...
def Camera_start(wx,hx,zoom):
    global sqpos,square,scientif,fxx,fxy,fxz,Pi_Cam,v3_f_modes,v3_f_mode,scientific,used_storage,trace,p,red,blue,contrast,brightness,gain,speed,modes,mode,ev,ES,cap_width,cap_height,pre_frames,awbs,awb,meters,meter,sharpness,saturation,denoise,cwidth,xheight
    if trace == 1:
    ...
    if mode == 0:
        rpistr +=" --shutter " + str(speed) + " --framerate " + str(fps)
    else:
    ...
    if square == 1:
        rpistr += " --roi " + str(sqpos) + ",0," + str(cap_height/cap_width) + ",1"
...
def main_menu():
    global ram_frames,frames,menu,ram_snaps,snaps,sd_hour,pf,vf,synced,Capture,show
    menu = -1
    show = 0
    Capture = old_cap
    for d in range(0,11):
...
while True:
    ...
    if show == 0:
        ...
        if np.shape(gray) == np.shape(oldimg):
                ...
                if used_storage > storage_limit :
                    ...
                    if len(USB_Files) > 0:
                        ...
                        for xx in range(0,len(zupics)):
                            ...
                            if menu == -1:
                                text(0,8,3,1,1,str(len(zupics)-xx),14,7)
                        frames = 0
                        vf = str(ram_frames) + " - " + str(frames)
                        if menu == -1:
                            text(0,0,3,1,1,vf,14,7)
            ...
            if ((sar5/diff) * 100 > detection and (sar5/diff) * 100 < det_high) or (time.monotonic() - timer10 > interval and timer10 != 0 and threshold == 0) or record == 1 :
                ...
                if Capture == 1 or record == 1:
                    ...
                    if ((ram_frames > 0 or frames > 0)  and menu == -1):
                        text(0,6,1,0,1,"SHOW, EDIT or",14,7)
                        text(0,6,1,1,1,"DELETE",14,7)
                    elif menu == -1:
                        text(0,6,0,0,1,"SHOW, EDIT or",14,7)
        ...
        if fcount > 0:
          ...
          if preview == 1 and np.shape(gray) == np.shape(oldimg) and zoom == 0:
          ...
          #crop3 = pygame.transform.scale(crop3, (h_crop*2,v_crop*2))
          #windowSurfaceObj.blit(crop3,( 0,0))
    ...
    for event in pygame.event.get():
        if (event.type == MOUSEBUTTONUP):
            ...
            # set crop position
            if mousex < xwidth and zoom == 0 and menu != 7:
               a = mousex
               b = mousey
               if a + h_crop > xwidth:
                   a = xwidth - h_crop
            ...
            # keys   
                ...
                if g == 0 and menu == -1 :
                    ...
                    # check for usb_stick and copy Video JPGs to it
                    USB_Files  = []
                    USB_Files  = (os.listdir("/media/" + h_user[0]))
                    text(0,9,3,0,1,"Copy JPGs",14,7)
                ...
                elif g == 0 and menu == 0 and zoom == 0:
                    # PREVIEW
                    preview +=1
                    if preview > 1:
                        preview = 0
                    ...
                    text(0,9,3,1,1,str(dspeed),14,7)
                    save_config = 1
                  ...
                  if len(Sideos) > 0:
                    ...
                    for xx in range(0,len(zpics)-1):
                      ...
                      if zpics[xx][40:45] != "99999":
                          if anno == 1:
                            cv2.putText(img,show_time,(100,image.get_height() - 50),cv2.FONT_HERSHEY_COMPLEX_SMALL,2,(255,255,255))
                          out.write(img)
                        #os.remove(zpics[xx])
                    out.release()
                 ...
                 if len(Sideos) > 0:
                  ...
                  if os.path.exists('/home/' + h_user[0] + '/Videos/' + outvids[0] + "_99999.jpg"):
                      image = pygame.image.load('/home/' + h_user[0] + '/Videos/' + outvids[0] + "_99999.jpg")
                  elif os.path.exists('/home/' + h_user[0] + '/Videos/' + outvids[0] + "_00001.jpg"):
                      image = pygame.image.load('/home/' + h_user[0] + '/Videos/' + outvids[0] + "_00001.jpg")
                  ...
                  pwm.ChangeDutyCycle(dc)
                    ...
                    if ((g == 7 and menu == -1) or (g == 9 and menu == 3)) and (ram_frames > 0 or frames > 0):
                        menu = 5
                        for d in range(0,10):
                            button(0,d,0)
                        show = 1
