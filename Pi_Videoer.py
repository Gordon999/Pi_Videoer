...
# external input trigger (if use_gpio = 1)
ext_trig = 26
...
if use_gpio == 1:
    ...
    GPIO.setup(ext_trig,GPIO.IN, pull_up_down=GPIO.PUD_UP)
...
h_crop        = 120     # size of hor detection window *
threshold     = 20      # minm change in pixel luminance
threshold2    = 100     # maxm change in pixel luminance
cap_width     = 1920
cap_height    = 1080
...
auto_limit    = 6       # No of Videos in RAM before auto save actioned
ram_limit     = 100     # MBytes, copy from RAM to SD card when reached
...
vformat       = 4       # 4 = 1920 X 1080, SEE VWIDTHS/VHEIGHTS
...
if not os.path.exists(config_file):
    points = [h_crop,threshold,fps,mode,speed,gain,brightness,contrast,Capture,preview,awb,detection,int(red*10),int(blue*10),interval,v_crop,v_length,ev,meter,ES,a,b,sharpness,saturation,denoise,fan_low,fan_high,vid_pic,det_high,quality,fan_time,sd_hour,vformat,threshold2,col_filter]
...
def Camera_start(wx,hx,zoom,vid_pic):
    global trace,p,red,blue,contrast,brightness,gain,speed,modes,mode,ev,ES,cap_width,cap_height,pre_frames,awbs,awb,meters,meter,sharpness,saturation,denoise,cwidth,xheight
    if trace == 1:
    ...
    text(0,10,3,1,1,str(int(freeram)),15,7)
...
while True:
    ...
    if show == 0:
        ...
        if np.shape(gray) == np.shape(oldimg):
            ...
            if menu == 0:
                text(0,9,3,1,1,str(np.max(ar5)),18,7)
            ar5[ar5 <  threshold] = 0
            ...
            if ((sar5/diff) * 100 > detection and (sar5/diff) * 100 < det_high) or (time.monotonic() - timer10 > interval and timer10 != 0 and threshold == 0) or record == 1 or GPIO.input(ext_trig) == 0:
                now = datetime.datetime.now()
        ...
        if zoom == 0:
            ...
            ta = (a * (cap_width/cwidth)) - (h_crop * (cap_width/cwidth))
            if ta + cwidth > cap_width:
                ta = cap_width - cwidth
            tb = (b * (cap_height/xheight)) - (v_crop * (cap_height/xheight))
            if tb + xheight > cap_height:
                tb = cap_height - xheight
            cropped_region = (ta,tb, cwidth, xheight)
               ...
               save_config = 1
                ...
                if g == 0 and menu == -1 :
                    ...
                    if (h == 1 and event.button == 1) or event.button == 4:
                        ...
                        fps = max(fps,5)
                    if mode != 0:
                        text(0,1,3,1,1,str(fps),18,7)
                        
                    else:
                        text(0,1,0,1,1,str(fps),18,7)
                    restart = 1
                    save_config = 1
                    ...
                    if (h == 1 and event.button == 1) or event.button == 4:
                        ...
                        awb -=1
                        awb = max(awb,0)
                    text(0,2,3,1,1,str(awbs[awb]),18,7)
                    if awb == 0:
                        text(0,3,3,1,1,str(red)[0:3],18,7)
                        text(0,4,3,1,1,str(blue)[0:3],18,7)
                    else:
                        text(0,3,0,1,1,str(red)[0:3],18,7)
                        text(0,4,0,1,1,str(blue)[0:3],18,7)
                    restart = 1
                    save_config = 1
                    
                elif g == 3 and menu == 2 and awb == 0:
                    # RED
                    restart = 1
                    ...
                    text(0,5,3,1,1,str(sharpness),18,7)
                    save_config = 1
                    ...
                    text(0,6,3,1,1,str(saturation),18,7)
                    save_config = 1
                    ...
                    text(0,7,3,1,1,str(denoises[denoise]),18,7)
                    save_config = 1
                    ...
                    text(0,8,3,1,1,str(quality),18,7)
                    save_config = 1
                    ...
                    text(0,8,3,1,1,str(v_length/1000),18,7)
                    save_config = 1
                 ...
                 if len(Sideos) > 0:
                  ...
                  pygame.display.update()
                ...
                elif (menu == -1 and g > 1 and g < 9) or (menu == -1 and g == 10) or (menu != -1 and g == 9):
                    # MENUS
                    # check for usb_stick
                    USB_Files  = []
                    USB_Files  = (os.listdir("/media/" + h_user[0] + "/"))
                    ...
                    if g == 4:
                        ...
                        text(0,0,3,1,1,str(vwidths[vformat]) + "x" + str(vheights[vformat]),18,7)
                        text(0,2,5,0,1,"AWB",14,7)
                        text(0,2,3,1,1,str(awbs[awb]),18,7)
                        text(0,1,5,0,1,"fps",14,7)
                        if mode == 0:
                            text(0,1,0,1,1,str(fps),18,7)
                        else:
                            text(0,1,3,1,1,str(fps),18,7)
                        text(0,3,5,0,1,"Red",14,7)
                        text(0,4,5,0,1,"Blue",14,7)
                        if awb == 0:
                            text(0,3,3,1,1,str(red)[0:3],18,7)
                            text(0,4,3,1,1,str(blue)[0:3],18,7)
                        else:
                            text(0,3,0,1,1,str(red)[0:3],18,7)
                            text(0,4,0,1,1,str(blue)[0:3],18,7)
                        text(0,5,5,0,1,"Sharpness",14,7)
                        text(0,5,3,1,1,str(sharpness),18,7)
                        text(0,6,5,0,1,"Saturation",14,7)
                        text(0,6,3,1,1,str(saturation),18,7)
                        text(0,7,5,0,1,"Denoise",14,7)
                        text(0,7,3,1,1,str(denoises[denoise]),18,7)
                        text(0,8,5,0,1,"Quality",14,7)
                        text(0,8,3,1,1,str(quality),18,7)
                        text(0,9,1,0,1,"MAIN MENU",14,7)
                        text(0,10,0,0,1,"Copy EXIT",14,7)
                    ...
                    if g == 6 and (((ram_frames > 0 or frames > 0) and vid_pic == 0) or ((ram_snaps > 0 or snaps > 0) and vid_pic == 1)):
                        ...
                        text(0,10,2,0,1,"Copy EXIT",14,7)
